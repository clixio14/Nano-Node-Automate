...
# 6a. Bandwidth test — skip if already done
if [ -f /home/nano-data/bandwidth_mbps.txt ]; then
  echo "# Bandwidth baseline already recorded, skipping"
else
  if command -v speedtest-cli &>/dev/null; then
    echo "# Checking your internet bandwidth — please wait..."
    SPEED_OUTPUT=$(speedtest-cli --simple 2>/dev/null)
    DL=$(echo "$SPEED_OUTPUT" | grep -oP 'Download:\s*\K[\d.]+' || echo "0")
    UL=$(echo "$SPEED_OUTPUT" | grep -oP 'Upload:\s*\K[\d.]+' || echo "0")
    echo "${DL} ${UL}" > /home/nano-data/bandwidth_mbps.txt
    chmod 644 /home/nano-data/bandwidth_mbps.txt
    echo "# Bandwidth recorded: ${DL} Mbps down / ${UL} Mbps up"
  else
    echo "# speedtest-cli not available — skipping bandwidth test"
  fi
fi

# 6b. Disk IOPS baseline test — skip if already done
# Runs 50 samples of random 4K writes — results saved for rep script and ping tool
if [ -f /home/nano-data/disk_iops.txt ]; then
  echo "# Disk IOPS baseline already recorded, skipping"
else
  if command -v ioping &>/dev/null; then
    echo "# Checking your system disk performance for Nano node smooth functionality"
    echo "# Please wait approximately 2 minutes..."
    IOPS_RAW=$(ioping -c 50 -s 4k -S 256M -W /home/nano-data 2>/dev/null | grep -oP '[\d.]+ k? ?iops' | tail -1)
    IOPS_VAL=$(echo "$IOPS_RAW" | python3 -c "
import sys, re
raw = sys.stdin.read().strip()
m = re.search(r'([\d.]+)\s*(k)?\s*iops', raw, re.IGNORECASE)
if m:
    val = float(m.group(1))
    if m.group(2):
        val *= 1000
...
# 17. Wrapper — 'rep' launches representative-setup.sh (downloaded when script is ready)
...
# 18. Wrapper — 'cap' launches bandwidth-cap-config.sh (downloaded when script is ready)

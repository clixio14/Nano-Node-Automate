...
# 2. Basic tools
# qrencode is used by representative-setup.sh to render the seed as a QR code on screen
# so users can scan it directly into a password manager app like Bitwarden or 1Password
# The seed is then permanently wiped from memory and screen once the user confirms backup
sudo apt install -y build-essential curl git qrencode

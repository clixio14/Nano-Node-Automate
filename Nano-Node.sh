...
# 7. Ledger Download — skip if already downloaded AND file passes integrity check
if [ -f /home/nano-data/Nano/Nano_Snapshot.7z ] && 7z t /home/nano-data/Nano/Nano_Snapshot.7z > /dev/null 2>&1; then
  echo "# Ledger snapshot already downloaded and verified, skipping"
else
...
# 8. Extraction — skip only if flag file confirms extraction completed successfully
if [ -f /home/nano-data/Nano/extraction_complete.flag ]; then
  echo "# Ledger already extracted and verified, skipping"
elif [ -f /home/nano-data/Nano/data.ldb ]; then

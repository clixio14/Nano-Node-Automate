...
get_network_block() {
  ...
  while IFS= read -r ep; do
    local count
    count=$(curl -sf --max-time 5 -X POST "$ep" \
...
")
    ...
    # Display values
    block_count_display="${BLUE}${node_block}${NC} (Your Node's Block)"
    nano_block_display="$net_block  (Latest Nano Network's Block)"
  ...
  echo "----------------------------------------------------------------------"
  if is_rpc_ready 2>/dev/null && [[ -n "$node_block" ]]; then

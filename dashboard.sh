...
if online > 0 and weight > 0:
    ...
    pct_str = '0%'
    voting_status = 'Yes (awaiting delegation)'
print(f'{nano_str}|{pct_str}|{voting_status}')
" 2>/dev/null || echo "0 XNO|0%|Yes (awaiting delegation)")
...
core_usage = max(0.5, round(perc / 100, 2))
...
")
  ...
  echo ""
  printf " %-22s %s\n"       "CPU Usage"          "$cpu_ghz"
  printf " %-22s %s\n"       "CPU Core Usage"     "$core_usage logical cores active / $total_threads logical ($physical_cores physical)"

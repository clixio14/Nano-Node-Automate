...
run_dashboard() {
  ...
  {
    docker ps --filter "name=nano-node" --format "Service Status|Running\nCreated Since|{{.RunningFor}}\nConcurrent Uptime|{{.Status}}" | sed 's/Up //'
    ...
    python3 -c "perc=$cpu_perc; cores=$physical_cores; threads=$total_threads; max_g='$max_ghz'; ram_str='$mem_usage'; sys_tot=$sys_total; lmdb='$lmdb_mem';
...
if max_g not in ('N/A', ''):
    ...
    cpu_ghz_str = 'N/A'
# Convert LMDB value from e.g. '5.8g' or '900m' to GB
...
print(f'CPU Usage|{cpu_ghz_str}\nCPU Core Usage|{core_usage} core / {cores} cores\nNode RAM Usage|{ram_str}\nLMDB Memory Map|{lmdb_gb:.2f} GB  (Ledger mapped into RAM by LMDB)\nPower Est. (Watts)|{total_w:.0f} Watts  (Incl. LMDB Overhead and I/O)')"
    ...
    echo "Internet Usage|$(echo "scale=2; ($rx2-$rx1)*8/1048576" | bc) Mbps Down / $(echo "scale=2; ($tx2-$tx1)*8/1048576" | bc) Mbps Up"
    ...
    echo "Node Version|V$v_maj.$v_min.$v_pat"

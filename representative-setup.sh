...
if [ "$COMMIT_LAT" != "skip" ]; then
  ...
  if [ "$COMMIT_LAT_INT" -lt 18 ] 2>/dev/null; then
    print_check "Block Commit Latency" "${COMMIT_LAT} ms  (fast — node will stay in sync)" "yes"
  else
    print_check "Block Commit Latency" "${COMMIT_LAT} ms  (too slow — node may fall behind in voting)" "no"
    SPECS_PASSED=false
  fi
fi

...
if [ "$IOPS_VAL" != "-1" ]; then
  if [ "$IOPS_VAL" -ge 700 ] 2>/dev/null; then
    print_check "Disk IOPS (random Write 4K)" "${IOPS_VAL} IOPS  (NVMe — excellent)" "yes"
  elif [ "$IOPS_VAL" -ge 301 ] 2>/dev/null; then
    print_check "Disk IOPS (random Write 4K)" "${IOPS_VAL} IOPS  (SSD/NVMe — good)" "yes"
  elif [ "$IOPS_VAL" -ge 100 ] 2>/dev/null; then
    print_check "Disk IOPS (random Write 4K)" "${IOPS_VAL} IOPS  (SSD — acceptable)" "yes"
  else
...
echo ""
echo -e " To see how your node compares to others installed by Nano Noder,"

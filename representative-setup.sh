...
if [ -f /home/nano-data/disk_iops.txt ]; then
  ...
  print_check "Disk IOPS (random Write 4K)" "Skipped (ioping unavailable)" "yes"
  IOPS_VAL="-1"
fi

if [ "$IOPS_VAL" != "-1" ]; then
  if [ "$IOPS_VAL" -ge 100 ] 2>/dev/null; then
    print_check "Disk IOPS (random Write 4K)" "${IOPS_VAL} IOPS" "yes"
  else
    print_check "Disk IOPS (random Write 4K)" "${IOPS_VAL} IOPS  (minimum 100 required — HDD detected)" "no"
    SPECS_PASSED=false
  fi
fi

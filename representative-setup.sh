...
SEED_FILE="$DATA_DIR/rep_seed.txt"
...
# ══════════════════════════════════════════════════════════════════════════════
#
#  ⚠️  TESTING ONLY — REMOVE THIS ENTIRE BLOCK BEFORE PRODUCTION RELEASE  ⚠️
#
#  This reset section destroys the previous representative wallet cleanly
#  from inside the node (rather than leaving orphaned wallets behind).
#  It is only here to make iterative testing easier without needing a
#  fresh Ubuntu install each time.
#
#  To remove when ready for production:
#  Delete from this comment down to the END OF TESTING BLOCK marker below.
#
# ══════════════════════════════════════════════════════════════════════════════
...
do_reset() {
  ...
  OLD_WALLET_ID=$(cat "$WALLET_ID_FILE")
  ...
  sudo rm -f "$WALLET_ID_FILE" "$REP_ADDRESS_FILE" "$SEED_FILE"
...
# Detect if a previous setup exists and offer to reset
if [ -f "$WALLET_ID_FILE" ] || [ -f "$REP_ADDRESS_FILE" ]; then
  clear
  echo ""
  echo "======================================================================"
  echo -e "${YELLOW}  A previous representative setup was detected.${NC}"
  echo "======================================================================"
  echo ""
  echo "  Saved files found:"
  [ -f "$WALLET_ID_FILE" ]   && echo -e "   ${CYAN}found${NC}  $WALLET_ID_FILE"
  [ -f "$REP_ADDRESS_FILE" ] && echo -e "   ${CYAN}found${NC}  $REP_ADDRESS_FILE"
  [ -f "$SEED_FILE" ]        && echo -e "   ${CYAN}found${NC}  $SEED_FILE"
  echo ""
  echo "  Continuing WITHOUT resetting will skip completed steps and resume"
  echo "  from where it left off."
  echo ""
  echo "  Resetting will cleanly remove the old wallet from the node and"
  echo "  start a completely fresh setup."
  echo ""
  echo -n "  Would you like to RESET and start completely fresh? [y/N]: "
  read -r DO_RESET
  echo ""
...
# ══════════════════════════════════════════════════════════════════════════════
#  ⚠️  END OF TESTING BLOCK — REMOVE EVERYTHING ABOVE THIS LINE  ⚠️
# ══════════════════════════════════════════════════════════════════════════════
...
echo    "     You will need to send at least 0.001 XNO to your new"
echo    "     representative address to activate it on the network."
echo ""
echo -e "  ${BOLD}3. Delegation is required to start voting${NC}"
echo    "     Your node needs at least 1001 XNO delegated to it (by you or others)"
...
if [ -f /home/nano-data/bandwidth_mbps.txt ]; then
  ...
  echo "  Internet speed (from setup baseline): ${DOWNLOAD_MBPS} Mbps down / ${UPLOAD_MBPS} Mbps up"
...
if [ "$DL_INT" -ge 100 ] && [ "$UL_INT" -ge 160 ]; then
  print_check "Internet Speed" "${DOWNLOAD_MBPS} Mbps down / ${UPLOAD_MBPS} Mbps up" "yes"
else
  print_check "Internet Speed" "${DOWNLOAD_MBPS} Mbps down / ${UPLOAD_MBPS} Mbps up  (need 100↓ / 160↑ Mbps)" "no"
  SPECS_PASSED=false
fi
...
# ── Disk IOPS (random 4K write — catches real HDDs) ──────────────────────────
if [ -f /home/nano-data/disk_iops.txt ]; then
  # Use cached result from Nano-Node.sh setup
  IOPS_VAL=$(cat /home/nano-data/disk_iops.txt)
  echo "  Disk IOPS (from setup baseline): ${IOPS_VAL} IOPS"
elif command -v ioping &>/dev/null; then
  # No cached result — run fresh 10-sample test
  echo "  Running disk IOPS test (this takes about 10 seconds)..."
  IOPS_VAL=$(ioping -c 10 -s 4k -S 256M -W /home/nano-data 2>/dev/null | grep 'requests completed' | python3 -c "import sys, re; line = sys.stdin.read(); m = re.search(r'([\d.]+)\s*(k)?\s*iops', line, re.IGNORECASE); print(int(float(m.group(1)) * (1000 if m.group(2) else 1)) if m else 0)" 2>/dev/null || echo "0")

else
  print_check "Disk IOPS (random 4K)" "Skipped (ioping unavailable)" "yes"
  IOPS_VAL="-1"
fi

if [ "$IOPS_VAL" != "-1" ]; then
  if [ "$IOPS_VAL" -ge 100 ] 2>/dev/null; then
    print_check "Disk IOPS (random 4K)" "${IOPS_VAL} IOPS" "yes"
  else
    print_check "Disk IOPS (random 4K)" "${IOPS_VAL} IOPS  (minimum 100 required — HDD detected)" "no"
    SPECS_PASSED=false
  fi
fi
...
if [ "$CPU_CORES" -ge 4 ]; then
  print_check "CPU Cores" "${CPU_CORES} cores" "yes"
else
  print_check "CPU Cores" "${CPU_CORES} cores  (minimum 4 required)" "no"
  SPECS_PASSED=false
fi
...
echo "   2. Create a secure representative wallet and account"
echo "   3. Save and optionally display your backup seed"
echo "   4. Activate your account (requires a tiny Nano deposit)"
echo "   5. Set this node to vote for itself"
echo "   6. Lock down your node for security"
echo ""
echo "======================================================================"
echo ""
...
# ─── Step 4: Back up the wallet seed ─────────────────────────────────────────
...
echo ""
echo "  We will always save a copy to a protected file on your server."
echo "  Only the root user (admin) can read that file."
echo ""
...
echo "$SEED" | sudo tee "$SEED_FILE" > /dev/null
sudo chmod 600 "$SEED_FILE"
sudo chown root:root "$SEED_FILE"

echo -e "  Seed saved to: ${CYAN}$SEED_FILE${NC}"
echo -e "  (Only accessible by the root/admin user)"
echo ""

echo -n "  Would you like to display your seed on screen to write it down? [y/N]: "
read -r SHOW_SEED
echo ""

if [[ "$SHOW_SEED" =~ ^[Yy]$ ]]; then
  echo -e "  ${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
  echo -e "  ${CYAN}║        YOUR SEED — WRITE THIS DOWN AND KEEP IT SAFE         ║${NC}"
  echo -e "  ${CYAN}╠══════════════════════════════════════════════════════════════╣${NC}"
  echo -e "  ${CYAN}║${NC}                                                              ${CYAN}║${NC}"
  echo -e "  ${CYAN}║${NC}  ${YELLOW}${SEED}${NC}  ${CYAN}║${NC}"
  echo -e "  ${CYAN}║${NC}                                                              ${CYAN}║${NC}"
  echo -e "  ${CYAN}╠══════════════════════════════════════════════════════════════╣${NC}"
  echo -e "  ${CYAN}║  NEVER share this with anyone. Store it offline safely.      ║${NC}"
  echo -e "  ${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
  echo ""
  echo -n "  Press ENTER once you have safely written down your seed: "
  read -r
else
  echo -e "  Skipped — your seed is safely stored at: ${CYAN}$SEED_FILE${NC}"
fi

echo -e "${GREEN}  Seed backup complete!${NC}"
...
if [ "$ACCOUNT_IS_OPEN" = "yes" ]; then
  ...
  echo ""
  echo -e "  ${BOLD}Please send at least 0.001 XNO (Nano) to this address:${NC}"
  echo ""
...
# ─── Step 7: Self-delegate (vote for itself) ──────────────────────────────────
print_step "Step 7 of 9 — Setting this node to vote for itself"

echo "  Configuring the wallet so that this node uses its own"
echo "  representative address when casting votes..."
echo ""

DELEGATE_RESPONSE=$(rpc "{\"action\":\"wallet_representative_set\",\"wallet\":\"$WALLET_ID\",\"representative\":\"$REP_ADDRESS\"}" 2>/dev/null)
SET_OK=$(echo "$DELEGATE_RESPONSE" | grep -oP '"set":\s*"\K[^"]+' || true)

if [ "$SET_OK" = "1" ]; then
  echo -e "${GREEN}  Self-delegation complete!${NC}"
  echo "  This node will now cast its voting weight using its own representative."
else
  echo -e "${YELLOW}  Self-delegation response: $DELEGATE_RESPONSE${NC}"
  echo -e "${YELLOW}  This is not critical — voting is still enabled. Continuing...${NC}"
fi

# ─── Step 8: Disable enable_control for security ─────────────────────────────
print_step "Step 8 of 9 — Locking down your node for security"
...
echo -e " ${CYAN}${REP_ADDRESS}${NC}"
echo ""
echo " Your seed backup is saved at:"
echo -e " ${CYAN}${SEED_FILE}${NC}"
echo " (Protected file — only the admin/root user can read it)"
echo ""
echo "----------------------------------------------------------------------"
echo -e "${YELLOW} NEXT STEP — To start voting, your node needs${NC}"
echo -e "${YELLOW} at least 1000 XNO delegated to it.${NC}"
echo ""
echo " Share your representative address with others and ask them"
echo " to set it as their representative in their Nano wallet."
echo " Every XNO they hold adds to your voting weight."
echo "----------------------------------------------------------------------"
echo ""
echo -e " Type ${CYAN}dashboard${NC} to monitor your node and see your voting stats."
echo ""
echo "======================================================================"
